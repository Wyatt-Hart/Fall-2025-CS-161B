...
int main(int argc, char *argv[]) {
  ...
  // The 2 arrays (double scores[], and char grades[]) must be declared in
  // main().
...
void welcome() { cout << "Welcome to my Parallel Arrays program!" << endl; }

// Reads a list of scores from the user. A -1 indicates the end of the input and
// is not stored in the array. (Use an appropriate loop!! Check out this example
// code - a for loop is count controlled, and a while loop is condition based.)
// Call the readDouble function to do this. You must catch all invalid data such
// as characters, negative numbers other than -1 etc. The variable count keeps
// track of the number of scores entered. You must also do validation to make
// sure that the number is between 0 and 4 inclusive and nothing other than
// that. Use a while loop to do this.
void readScores(double scores[], int &count) {
  double num;
...
// This function should be used any time you read any floats or doubles from the
// user. Use this function to read the numerical scores from the user. It takes
// a string prompt, outputs it, reads a number from the user, validates and
// returns the num by reference. Write it exactly like the readInt from
// assignment 1 but declare a double or float instead of an int. See
// Samplea01.cpp for the readInt function. You must catch all invalid data such
// as characters, negative numbers etc.
void readDouble(string prompt, double &num) {
  cout << prompt;
  cin >> num;
  while (!cin) {
...
// Use the table below to assign grades A to F to the corresponding numerical
// score.
...
// Use a loop to process each array element and calculate the letter grade for
// each score.
void calcGrade(double scores[], char grades[], int count) {
...
// Go through a for loop and print the scores and the corresponding grades for
// each item.
void printList(double scores[], char grades[], int count) {
  for (int i = 0; i < count; i++) {
...
// Sort the arrays using the given sorting algorithm. This is called Selection
// Sort. Use only this algorithm to sort your list. To see how the Selection
// Card Sort Algorithm works, watch this video from Virginia Tech. Be careful
// and make sure you sort based on the scores array and swap the corresponding
// element in the grade array to maintain the correspondence between the two
// arrays. Meaning if you swap the scores in index 0 and 5, you must also swap
// the corresponding grades in index 0 and 5.
void sort(double scores[], char grades[], int count) {
  for (int i = 0; i < count; i++) {
    int min = i;
...
// Otherwise, the median is the average of the middle two values. Return the
// median to main() and output in main() with two decimal places.
double median(double scores[], int count) {
  if (count % 2 == 0) {
...
Program Description:
In this assignment, you will be writing a program that stores a list of scores
and the corresponding letter grades in two parallel arrays. The scores will be
double data type and the letter grade will be a char data type.
